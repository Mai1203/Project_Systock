import sys
import sqlite3  # Importación corregida
...
class LoginWindow(QDialog):
    ...
    def check_credentials(self):
        ...
        # Conectar a SQLite y validar las credenciales
        connection = sqlite3.connect("usuarios.db")
        cursor = connection.cursor()

        query = "SELECT * FROM usuarios WHERE rol = ? AND contraseña = ?"
        cursor.execute(query, (role, password))
        result = cursor.fetchone()

        connection.close()

        if result:
            print(f"Login como {role} exitoso")
